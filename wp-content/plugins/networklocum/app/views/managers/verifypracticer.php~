...
jQuery(document).ready(function() {
	...
	jQuery("#submit1").click(function () {
		$("#verifiedpracticer").val(1);		
		$("#verifypracticer").submit();
  	});

	jQuery("#submit2").click(function () {
		$("#verifiedpracticer").val(2);	
		$("#verifypracticer").submit();
	});
 });
</script>
...
<h2 class="text1"> Verfiy practicer profile </h2>
</div>
<?php
/*
...
?> 
				 ...
				 <div align="center">
							<form name="verifypracticer" id="verifypracticer"   autocomplete="off"  method="post" action="<?php echo $url;?>managers/verifypracticer"  enctype="multipart/form-data">

						<input type="hidden" name="verifiedpracticer" id="verifiedpracticer" value="0"/>
						<input type="hidden" name="id" id="id" value="<?php echo $practicerobject->id;?>"/>
 
					<?php
					if( $practicerobject->verifiedpracticer == 0) { ?>
						<button class="btn btn-info sbtn" id="submit1">Verify</button>
						<button class="btn btn-info sbtn" id="submit2">Reject</button>
					<?php } ?>			
					</form>
				</div>
